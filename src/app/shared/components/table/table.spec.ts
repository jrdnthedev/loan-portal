...
describe('Table', () => {
  ...
  describe('With Columns Configuration', () => {
    ...
    it('should render configured column headers', () => {
      const headerCells = hostDebugElement.queryAll(By.css('th'));
    ...
    it('should render data in correct column order', () => {
      const dataRows = hostDebugElement.queryAll(By.css('tbody tr'));
      expect(dataRows.length).toBe(2);
      ...
      const secondRowCells = dataRows[1].queryAll(By.css('td'));
      expect(secondRowCells[0].nativeElement.textContent.trim()).toBe('2');
      expect(secondRowCells[1].nativeElement.textContent.trim()).toBe('Jane Smith');
      expect(secondRowCells[2].nativeElement.textContent.trim()).toBe('jane@example.com');
      expect(secondRowCells[3].nativeElement.textContent.trim()).toBe('inactive');
    });

    it('should use column labels for data-label attributes', () => {
      const firstRowCells = hostDebugElement.queryAll(By.css('tbody tr:first-child td'));
      expect(firstRowCells[0].nativeElement.getAttribute('data-label')).toBe('ID');
      expect(firstRowCells[1].nativeElement.getAttribute('data-label')).toBe('Full Name');
      expect(firstRowCells[2].nativeElement.getAttribute('data-label')).toBe('Email Address');
      expect(firstRowCells[3].nativeElement.getAttribute('data-label')).toBe('Status');
    });
  });

  describe('Auto-Generated Headers', () => {
    let hostComponent: AutoGenerateHostComponent;
    let hostFixture: ComponentFixture<AutoGenerateHostComponent>;
    let hostDebugElement: DebugElement;

    beforeEach(async () => {
      await TestBed.configureTestingModule({
        imports: [Table],
        declarations: [AutoGenerateHostComponent],
      }).compileComponents();

      hostFixture = TestBed.createComponent(AutoGenerateHostComponent);
      hostComponent = hostFixture.componentInstance;
      hostDebugElement = hostFixture.debugElement;
      hostFixture.detectChanges();
    });
    ...
    it('should render auto-generated data correctly', () => {
      const dataRows = hostDebugElement.queryAll(By.css('tbody tr'));
      expect(dataRows.length).toBe(2);
      ...
      expect(firstRowCells[2].nativeElement.textContent.trim()).toBe('30');
    });

    it('should use keys as data-label attributes', () => {
      const firstRowCells = hostDebugElement.queryAll(By.css('tbody tr:first-child td'));
      expect(firstRowCells[0].nativeElement.getAttribute('data-label')).toBe('firstName');
      expect(firstRowCells[1].nativeElement.getAttribute('data-label')).toBe('lastName');
      expect(firstRowCells[2].nativeElement.getAttribute('data-label')).toBe('age');
    });
  });
